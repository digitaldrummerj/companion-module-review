...
Read `.claude/skills/review-scorecard/SKILL.md` for the 📊 Scorecard and 📋 Issues format. Merge the Step-4 deterministic findings + the subagents' findings into ONE file, deduped by file+line. Header meta table includes a **`Scope:`** line (`tag` / `module` / `both`). Sections in order: title + meta, **Fix Summary for Maintainer**, **📊 Scorecard**, **Verdict**, **📋 Issues** (Blocking / Non-blocking), `🔴 Critical` → `🟠 High` → `🟡 Medium` → `🟢 Low` → `💡 Nice to Have` → `🔮 Next Release` → `⚠️ Pre-existing Notes` → `🧪 Tests` → `✅ What's Solid`. Omit empty sections. Plain text (no emoji) in individual issue headings for stable anchors.
